...
// Markdown bold syntax pattern: **text** (including text with CJK characters)
const MARKDOWN_BOLD = /\*\*[^*\n]+?\*\*/g;
...
class Pangu {
  ...
  spacing(text: string): string {
    ...
    // 為了避免「網址」和「Markdown 粗體語法」被加入了盤古之白，所以要從轉換名單中剔除
    let index = 0;
    const matchUrls: string[] = []; // 存储原始网址
    const matchBolds: string[] = []; // 存储原始粗體語法
    ...
    // 保護 Markdown 粗體語法
    newText = newText.replace(MARKDOWN_BOLD, (match) => {
      matchBolds.push(match); // 将匹配的粗體語法存入数组
      return `{${index++}}`;
    });
    ...
    // 還原網址和粗體語法
    const allMatches = [...matchUrls, ...matchBolds];
    newText = newText.replace(/{\d+}/g, (match) => {
      const number = parseInt(match.match(/\d+/)![0]);
      // Only restore if this is a valid placeholder index
      if (number < allMatches.length && allMatches[number] !== undefined) {
        return allMatches[number];
      }
      // Keep the original text if it's not a valid placeholder (e.g., LaTeX formulas)
      return match;
    });
